...
class DerivedPropertyTests(unittest.TestCase):
    ...
    def test_derived_property(self):
        ...
        self.upsert_ratings_property("BBG000HC9638", fitch_value=10, moodys_value=5)
        self.upsert_ratings_property("BBG009LHM8H9")
        self.upsert_ratings_property("BBG001CY5LY0", moodys_value=5)
        self.upsert_ratings_property("BBG000GJZW38", fitch_value=10)
        ...
        moodys_then_fitch = self.get_instruments_with_derived_prop("BBG000HC9638")
        self.assertEqual(moodys_then_fitch, 5.0)

        # test case for derived property with no ratings
        no_ratings = self.get_instruments_with_derived_prop("BBG009LHM8H9")
        self.assertEqual(no_ratings, 0.0)

        # # test case for derived property with fitch only
        fitch_only = self.get_instruments_with_derived_prop("BBG000GJZW38")
        self.assertEqual(fitch_only, 10.0)

        # test case for derived property with moodys only
        moodys_only = self.get_instruments_with_derived_prop("BBG001CY5LY0")
