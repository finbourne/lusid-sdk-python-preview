...
class Valuation(unittest.TestCase):
    ...
    def test_aggregation_inline_recipe(self) -> None:
        ...
        for item in aggregation.data:
            print(
                f"\t{item['Instrument/default/Name']}\t{item['Proportion(Holding/default/PV)']}\t{item['Sum(Holding/default/PV)']}"
            )
    ...
    def test_aggregation_with_recipe_id(self) -> None:
        """
        Test valuation with an aggregation request using an upserted recipe
        """
        ...
        aggregation_request = self.create_aggregation_request(
            recipe_scope=self.recipe_scope, recipe_code=self.recipe_code
        )
