...
class OrderRequest(object):
    ...
    def side(self, side):
        ...
        if side is not None and len(side) > 512:
            raise ValueError("Invalid value for `side`, length must be less than or equal to `512`")  # noqa: E501
        if side is not None and len(side) < 0:
            raise ValueError("Invalid value for `side`, length must be greater than or equal to `0`")  # noqa: E501
        if (side is not None and not re.search(r'(?s).*', side)):  # noqa: E501
            raise ValueError(r"Invalid value for `side`, must be a follow pattern or equal to `/(?s).*/`")  # noqa: E501
