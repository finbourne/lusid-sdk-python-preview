...
class StructuredResultData(object):
    ...
    def document_format(self, document_format):
        ...
        if document_format is not None and len(document_format) > 512:
            raise ValueError("Invalid value for `document_format`, length must be less than or equal to `512`")  # noqa: E501
        if document_format is not None and len(document_format) < 1:
            raise ValueError("Invalid value for `document_format`, length must be greater than or equal to `1`")  # noqa: E501
        if (document_format is not None and not re.search(r'(?s).*', document_format)):  # noqa: E501
            raise ValueError(r"Invalid value for `document_format`, must be a follow pattern or equal to `/(?s).*/`")  # noqa: E501
