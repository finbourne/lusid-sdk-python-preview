...
class DataTypesApi(object):
    ...
    def update_data_type_with_http_info(self, scope, code, **kwargs):  # noqa: E501
        ...
        if ('scope' in local_var_params and
                len(local_var_params['scope']) > 100):
            raise ApiValueError("Invalid value for parameter `scope` when calling `update_data_type`, length must be less than or equal to `100`")  # noqa: E501
        if ('scope' in local_var_params and
                len(local_var_params['scope']) < 1):
        ...
        if ('code' in local_var_params and
                len(local_var_params['code']) > 100):
            raise ApiValueError("Invalid value for parameter `code` when calling `update_data_type`, length must be less than or equal to `100`")  # noqa: E501
        if ('code' in local_var_params and
                len(local_var_params['code']) < 1):
